<!DOCTYPE html>

<html lang="en">

<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Cottage Package - Rivertern Lodge Bhadra</title>
    <meta name="description"
        content="Explore the Cottage Package at Rivertern Lodge Bhadra, a tranquil retreat in Karnataka's Chikmagalur district.">
    <meta name="keywords"
        content="River Tern Lodge Bhadra, Cottage Package, Accommodation, Karnataka, Chikmagalur, Eco-Lodge, Nature Retreat, Adventure, Wildlife Sanctuary">
    <meta name="author" content="Rivertern Lodge Bhadra">
    <link href="assets/img/logos/favicon.webp" rel="shortcut icon" type="image/png">
    <link href="assets/css/theme-1.min.css" rel="stylesheet">
    <link href="assets/css/theme-2.min.css" rel="stylesheet">
    <link href="assets/css/theme-3.min.css" rel="stylesheet">
    <link rel="canonical" href="https://riverternlodgebhadra.com/cottage.php" />
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/flatpickr/dist/flatpickr.min.css">
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/flatpickr/dist/themes/dark.css">
    <script src="https://www.google.com/recaptcha/api.js" async defer></script>
</head>

<!-- /Head -->

<body>
    <!-- Preloader -->

    <div id="preloader" class="skeleton-wrapper">
        <!-- Navigation skeleton -->
        <nav class="nav-skeleton">
            <div class="logo-skeleton shimmer"></div>
            <div class="menu-skeleton">
                <div class="menu-item shimmer"></div>
                <div class="menu-item shimmer"></div>
                <div class="menu-item shimmer"></div>
                <div class="menu-item shimmer"></div>
            </div>
            <div class="cta-skeleton">
                <div class="phone-skeleton shimmer"></div>
                <div class="button-skeleton shimmer"></div>
            </div>
        </nav>

        <!-- Hero section skeleton -->
        <div class="hero-skeleton">
            <div class="small-text-skeleton shimmer"></div>
            <div class="large-text-skeleton-2 shimmer"></div>
            <div class="large-text-skeleton shimmer"></div>
            <div class="large-text-skeleton-2 shimmer"></div>
            <div class="button-skeleton shimmer"></div>
        </div>
    </div>

    <!-- /Preloader -->

    <!-- Header -->
    <header class="header">
        <div class="header_in ">



            <div class="logo-section">
                <a class="navbar-brand" href="index.php">
                    <img height="100%" sizes="(max-width: 600px) 480px, (max-width: 992px) 768px, 1200px" src="assets/img/logos/Logo.webp" alt="RiverTern">
                </a>
            </div>

            <ul class="navbar-nav">

                <li class="nav-item dropdown">
                    <a class="nav-link dropdown-toggle" id="navbarDropdown" role="button" data-bs-toggle="dropdown"
                        aria-expanded="false">
                        <span>ACCOMMODATION</span>
                    </a>
                    <ul class="dropdown-menu slideIn" aria-labelledby="navbarDropdown">
                        <li>
                            <a class="dropdown-item" href="loghut.php
">Loghut Package</a>
                        </li>
                        <li>
                            <a class="dropdown-item" href="cottage.php">Cottage Package</a>
                        </li>

                    </ul>
                </li>
                <li class="nav-item">
                    <a class="nav-link" href="gallery.php"><span>PHOTO GALLERY</span></a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" href="things-to-do.php"><span>THINGS TO DO</span></a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" href="reach.php"><span>HOW TO REACH</span></a>
                </li>

            </ul>
            <div class="cta-btn ">
                <a class="btn    mnw-180" href="tel:+9480022108">

                    <i class="hicon hicon-bold hicon-telephone"></i>
                    <span>+91-9480022108</span>

                </a>
            </div>
            <div class="book-btn ">
                <a href="booking.php">
                    <button type="submit" class="btn btn-primary btn-uppercase w-100">
                        <i class="hicon hicon-bold hicon-menu-bookings"></i>
                        <span>PLAN YOUR TRIP</span>
                    </button>
                </a>
            </div>


            <button type="button" class="toggle ">
                <i class="hicon hicon-bold hicon-hamburger-menu"></i>
            </button>
            <button type="button" class="btn-close ">
            </button>

        </div>
    </header>
    <!-- /Header -->

    <!-- Sidebar -->
    <div class="sidebar" id='sidebar'>


        <ul class="mobile-nav-link p-0">

            <li class="nav-item dropdown">

                <a class="nav-link dropdown-toggle" id="navbarDropdown" role="button" data-bs-toggle="dropdown"
                    aria-expanded="false">
                    <i class="hicon hicon-bold hicon-agoda-price-guarante"></i> <span>ACCOMMODATION</span>
                </a>
                <ul class="dropdown-menu slideIn" aria-labelledby="navbarDropdown">
                    <li>
                        <a class="dropdown-item" href="loghut.php
">Loghut Package</a>
                    </li>
                    <li>
                        <a class="dropdown-item" href="cottage.php">Cottage Package</a>
                    </li>

                </ul>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="gallery.php"><i class="hicon hicon-bold hicon-camera-hover"></i><span>PHOTO
                        GALLERY</span></a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="things-to-do.php"><i
                        class="hicon hicon-bold hicon-regular-hiking"></i><span>THINGS TO DO</span></a>
            </li>
            <li class="nav-item border-bottom">
                <a class="nav-link" href="reach.php"> <i class="hicon hicon-bold hicon-map-entry"></i><span>HOW TO
                        REACH</span></a>
            </li>
            <li class="nav-item mt-4">

                <a class="nav-link" href="tel:+9480022108">
                    <i class="hicon hicon-bold hicon-telephone"></i>
                    <span>+91-9480022108</span>
                </a>
            </li>


        </ul>

        <a href="booking.php">
            <button type="submit" class="btn text-white btn-primary btn-uppercase w-100">
                <i class="hicon hicon-bold hicon-menu-bookings"></i>
                <span>PLAN YOUR TRIP</span>
            </button>
        </a>

    </div>
    <!-- /Sidebar -->


    <!-- Main -->
    <main>

        <!-- Title -->
        <section class=" hero sub-page-hero">
            <div class="hero-bg">
                <div class="image-cover"></div>
            </div>
            <div class="bg-content container">
                <div class="hero-page-title">
                    <span class="hero-sub-title">Charming Haven</span>
                    <h1 class="display-3 hero-title">
                        Cottage Package
                    </h1>
                </div>
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0">
                        <li class="breadcrumb-item"><a href="index.php">Home</a></li>

                        <li class="breadcrumb-item active" aria-current="page">Cottage Package</li>
                    </ol>
                </nav>
            </div>
        </section>
        <!-- /Title -->




        <!-- Tour details -->
        <section class="p-top-90 p-bottom-90 bg-gray-gradient">

            <div class="container">
                <div class="d-lg-flex align-items-lg-end pb-5">
                    <div class="block-title me-auto">
                        <small class="sub-title">Rivertern Lodge Bhadra</small>
                        <h2 class="display-5 title">Accommodation - Cottage Package</h2>
                    </div>

                </div>
                <div class="row g-0">
                    <div class="col-12 col-xxl-9 col-xl-8">
                        <!-- Tour -->
                        <div class="pe-xxl-4 me-xxl-2">
                            <!-- Photos -->
                            <div class="card border-0 shadow-sm p-xl-2 mb-4">
                                <div class="card-body">
                                    <div class="border-bottom mb-4 pb-4">
                                        <h3 class="text-body-emphasis mb-0">
                                            Photos
                                        </h3>
                                    </div>
                                    <div class="row g-1 align-items-center">
                                        <!-- Large Image (Left Side, Spanning Two Rows) -->
                                        <div class="col-6 col-md-6 d-flex flex-column">
                                            <figure class="image-hover rounded mb-1">
                                                <img width="100%" height="100%" sizes="(max-width: 600px) 480px, (max-width: 992px) 768px, 1200px" src="assets/img/destinations/hello-4.webp" alt="RiverTern"
                                                    class="img-fluid">
                                            </figure>
                                            <div class="row g-1">
                                                <div class="col-6 col-lg-6">
                                                    <figure class="image-hover rounded mb-0">
                                                        <img width="100%" height="100%" sizes="(max-width: 600px) 480px, (max-width: 992px) 768px, 1200px" src="assets/img/destinations/hello5.webp" alt="RiverTern"
                                                            class="img-fluid">
                                                    </figure>
                                                </div>
                                                <div class="col-6 col-lg-6">
                                                    <figure class="image-hover rounded mb-0">
                                                        <img width="100%" height="100%" sizes="(max-width: 600px) 480px, (max-width: 992px) 768px, 1200px" src="assets/img/destinations/hello6.webp" alt="RiverTern"
                                                            class="img-fluid">
                                                    </figure>
                                                </div>
                                            </div>
                                        </div>

                                        <!-- Right Side (2x2 Grid with Four Images) -->
                                        <div class="col-6 col-md-6">
                                            <div class="row g-1">
                                                <div class="col-6 col-lg-6">
                                                    <figure class="image-hover rounded mb-0">
                                                        <img width="100%" height="100%" sizes="(max-width: 600px) 480px, (max-width: 992px) 768px, 1200px" src="assets/img/destinations/hello2.webp" alt="RiverTern"
                                                            class="img-fluid">
                                                    </figure>
                                                </div>
                                                <div class="col-6 col-lg-6">
                                                    <figure class="image-hover rounded mb-0">
                                                        <img width="100%" height="100%" sizes="(max-width: 600px) 480px, (max-width: 992px) 768px, 1200px" src="assets/img/destinations/home-5.webp" alt="RiverTern"
                                                            class="img-fluid">
                                                    </figure>
                                                </div>
                                                <div class="col-6 col-lg-6">
                                                    <figure class="image-hover rounded mb-0">
                                                        <img width="100%" height="100%" sizes="(max-width: 600px) 480px, (max-width: 992px) 768px, 1200px" src="assets/img/destinations/home-3.webp" alt="RiverTern"
                                                            class="img-fluid">
                                                    </figure>
                                                </div>
                                                <div class="col-6 col-lg-6">
                                                    <figure class="image-hover rounded mb-0">
                                                        <img width="100%" height="100%" sizes="(max-width: 600px) 480px, (max-width: 992px) 768px, 1200px" src="assets/img/destinations/home-4.webp" alt="RiverTern"
                                                            class="img-fluid">
                                                    </figure>
                                                </div>
                                                <div class="col-12 col-lg-12">
                                                    <figure class="image-hover rounded mb-0">
                                                        <img width="100%" height="100%" sizes="(max-width: 600px) 480px, (max-width: 992px) 768px, 1200px" src="assets/img/destinations/pkg-long.webp" alt="RiverTern"
                                                            class="img-fluid">
                                                    </figure>
                                                </div>
                                            </div>
                                        </div>
                                    </div>


                                </div>
                            </div>
                            <!-- /Photos -->
                            <!-- Featured -->
                            <ul class="list-unstyled row g-3 mb-4">
                                <li class="col-6 col-xl-4">
                                    <div class="mini-card pkg-card shadow-sm rounded">
                                        <span class="card-icon">
                                            <i class="hicon hicon-car-park-free-charge"></i>
                                        </span>
                                        <div class="card-content">
                                            <small class="card-desc">Parking</small>
                                            <h3 class="card-title">Free</h3>
                                        </div>
                                    </div>
                                </li>
                                <li class="col-6 col-xl-4">
                                    <div class="mini-card pkg-card shadow-sm rounded">
                                        <span class="card-icon">
                                            <i class="hicon hicon-breakfast-buffet"></i>
                                        </span>
                                        <div class="card-content">
                                            <small class="card-desc">Food</small>
                                            <h3 class="card-title">Buffet</h3>
                                        </div>
                                    </div>
                                </li>
                                <li class="col-6 col-xl-4">
                                    <div class="mini-card pkg-card shadow-sm rounded">
                                        <span class="card-icon">
                                            <i class="hicon hicon-map-entry"></i>
                                        </span>
                                        <div class=" card-content">
                                            <small class="card-desc">Location</small>
                                            <h3 class="card-title">Bhadra</h3>
                                        </div>
                                    </div>
                                </li>
                                <li class="col-6 col-xl-4">
                                    <div class="mini-card pkg-card shadow-sm rounded">
                                        <span class="card-icon">
                                            <i class="hicon hicon-time-clock"></i>
                                        </span>
                                        <div class="card-content">
                                            <small class="card-desc">Trip Duration</small>
                                            <h3 class="card-title">1 Night/2 Days</h3>
                                        </div>
                                    </div>
                                </li>
                                <li class="col-6 col-xl-4">
                                    <div class="mini-card pkg-card shadow-sm rounded">
                                        <span class="card-icon">
                                            <i class="hicon hicon-regular-hiking"></i>
                                        </span>
                                        <div class="card-content">
                                            <small class="card-desc">Type</small>
                                            <h3 class="card-title">Adventure</h3>
                                        </div>
                                    </div>
                                </li>
                                <li class="col-6 col-xl-4">
                                    <div class="mini-card pkg-card shadow-sm rounded">
                                        <span class="card-icon">
                                            <i class="hicon hicon-right-tick"></i>
                                        </span>
                                        <div class="card-content">
                                            <small class="card-desc">Guide</small>
                                            <h3 class="card-title">Yes</h3>
                                        </div>
                                    </div>
                                </li>
                            </ul>
                            <!-- /Featured -->
                            <!-- About -->
                            <div class="card border-0 shadow-sm p-xl-2 mb-4">
                                <div class="card-body">
                                    <div class="border-bottom mb-4 pb-4">
                                        <h3 class="text-body-emphasis mb-0">About</h3>
                                    </div>
                                    <div class="d-lg-flex border-bottom pb-3 mb-4">
                                        <div class="mnw-200">
                                            <h3 class="h5">Overview</h3>
                                        </div>
                                        <p>Are you dreaming of a getaway where nature takes center stage? The Cottage
                                            Package at River Tern Lodge in Bhadra is your perfect escape. Nestled on a
                                            hillock, these cozy cottages come with private verandas offering
                                            breathtaking views of the Bhadra Reservoir. It's like having a front-row
                                            seat to nature's finest show. With hearty buffet meals served in the lodge's
                                            charming Gol Ghar dining area, this package ensures you're well-fed and
                                            utterly relaxed throughout your stay.</p>
                                    </div>
                                    <div class="d-lg-flex">
                                        <div class="mnw-200">
                                            <h3 class="h5">Experience</h3>
                                        </div>
                                        <p>
                                            Imagine waking up to misty mornings and birds singing, stepping onto your
                                            veranda to soak in the peaceful surroundings. With this package, it's not
                                            just about the accommodation—it's about the adventure! Explore the Bhadra
                                            Tiger Reserve on exciting jeep and boat safaris, or dive into thrilling
                                            activities like kayaking and swimming. When the day winds down, gather
                                            around a campfire or enjoy captivating wildlife documentaries. This is more
                                            than just a trip; it's an experience to treasure!
                                        </p>
                                    </div>
                                </div>
                            </div>
                            <!-- /About -->

                            <div class="row g-4 mb-4">
                                <div class="col-12 col-xxl-6 col-lg-6">
                                    <!-- Weekday Package Pricing -->
                                    <div class="horizontal-review pkg-price-box rounded border h-100">
                                        <h3 class="h4 review-title pb-4 mb-4 lh-base">Weekday Package </h3>
                                        <div class="review-content">
                                            <h3 class="review-score">₹ 7,930</h3>
                                            <div class="review-total">
                                                <span class="star-rate-view"><span
                                                        class="star-value rate-50"></span></span>
                                                <span><strong class="text-body">Per Person</strong></span>
                                            </div>
                                        </div>
                                        <div class="review-label">
                                            <small>Mon to Thu</small>
                                        </div>
                                    </div>
                                    <!-- /Weekday Package Pricing -->
                                </div>
                                <div class="col-12 col-xxl-6 col-lg-6">
                                    <!-- Weekend Package Pricing -->
                                    <div class="horizontal-review pkg-price-box rounded border h-100">
                                        <h3 class="h4 review-title pb-4 mb-4 lh-base">Weekend Package</h3>
                                        <div class="review-content">
                                            <h3 class="review-score">₹ 9,330</h3>
                                            <div class="review-total">
                                                <span class="star-rate-view"><span
                                                        class="star-value rate-50"></span></span>
                                                <span><strong class="text-body">Per Person</strong></span>
                                            </div>
                                        </div>
                                        <div class="review-label">
                                            <small>Fri to Sun</small>
                                        </div>
                                    </div>
                                    <!-- /Weekend Package Pricing -->
                                </div>

                            </div>

                            <!-- Itinerary -->
                            <div class="card border-0 shadow-sm p-xl-2 mb-4">
                                <div class="card-body">
                                    <div class="border-bottom mb-4 pb-4">
                                        <h3 class="text-body-emphasis mb-0">
                                            Itinerary
                                        </h3>
                                    </div>
                                    <p>
                                        Get ready for an exciting 2-day adventure at River Tern Lodge in Bhadra! Enjoy
                                        stunning coastal views, hike through majestic mountains, and dive into thrilling
                                        water sports. Encounter diverse wildlife in lush tropical forests and immerse
                                        yourself in the local culture. Check out the detailed itinerary below for a
                                        perfect blend of adventure and natural beauty!
                                    </p>


                                    <div class="table-container">

                                        <table>

                                            <thead>

                                                <tr>

                                                    <th colspan="2">Day - 1</th>

                                                </tr>

                                            </thead>

                                            <tbody>



                                                <tr>

                                                    <td>
                                                        <p>01:00 PM</p>
                                                    </td>

                                                    <td>
                                                        <p>Check-in</p>
                                                    </td>

                                                </tr>

                                                <tr>

                                                    <td>
                                                        <p>01:30 PM - 02:30 PM</p>
                                                    </td>

                                                    <td>
                                                        <p>Buffet Lunch</p>
                                                    </td>

                                                </tr>

                                                <tr>

                                                    <td>
                                                        <p>03:00 PM - 03:15 PM</p>
                                                    </td>

                                                    <td>
                                                        <p>Safari briefing with tea/coffee</p>
                                                    </td>

                                                </tr>

                                                <tr>

                                                    <td>
                                                        <p>03:15 PM - 06:15 PM</p>
                                                    </td>

                                                    <td>
                                                        <p>Van Safari into Nagarahole National Park.</p>
                                                    </td>

                                                </tr>

                                                <tr>

                                                    <td>
                                                        <p>06:15 PM - 07:00 PM</p>
                                                    </td>

                                                    <td>
                                                        <p>Wildlife film/slide show</p>
                                                    </td>

                                                </tr>

                                                <tr>

                                                    <td>
                                                        <p>07:30 PM - 08:15 PM</p>
                                                    </td>

                                                    <td>
                                                        <p>Buffet Dinner Around Camp-Fire At The ‘Gol Ghar’</p>
                                                    </td>

                                                </tr>

                                                <tr>

                                                    <td>
                                                        <p>08:30 PM - 10:00 PM</p>
                                                    </td>

                                                    <td>
                                                        <p>Dinner</p>
                                                    </td>

                                                </tr>

                                            </tbody>

                                        </table>

                                    </div>
                                    <div class="table-container">

                                        <table>

                                            <thead>

                                                <tr>

                                                    <th colspan="2">Day - 2</th>

                                                </tr>

                                            </thead>

                                            <tbody>

                                                <tr>

                                                    <td>
                                                        <p>05:45 AM - 06:15 AM</p>
                                                    </td>

                                                    <td>
                                                        <p>Wake-up call, tea/coffee</p>
                                                    </td>

                                                </tr>

                                                <tr>

                                                    <td>
                                                        <p>06:15 AM - 09:30 AM</p>
                                                    </td>

                                                    <td>
                                                        <p>Drive into the Park, Boat Safari on River Kabini</p>
                                                    </td>

                                                </tr>

                                                <tr>

                                                    <td>
                                                        <p>09:30 AM - 10:15 AM</p>
                                                    </td>

                                                    <td>
                                                        <p>
                                                            Breakfast At The Gol-Ghar

                                                        </p>
                                                    </td>

                                                </tr>

                                                <tr>

                                                    <td>
                                                        <p>10:30 AM</p>
                                                    </td>

                                                    <td>
                                                        <p>Check-out</p>
                                                    </td>

                                                </tr>

                                            </tbody>

                                        </table>

                                    </div>

                                </div>
                            </div>
                            <!-- /Itinerary -->
                            <!-- Policy -->
                            <div class="card border-0 shadow-sm p-xl-2 mb-4">
                                <div class="card-body">
                                    <div class="border-bottom mb-4 pb-4">
                                        <h3 class="text-body-emphasis mb-0">
                                            Policy
                                        </h3>

                                    </div>
                                    <p> <a href="cancellation.php">Click here </a>to review the cancellation and
                                        postponement policy.
                                    </p>

                                </div>
                            </div>
                            <!-- /Policy -->
                            <!-- Reviews -->

                            <div class="card border-0 mb-3 shadow-sm p-xl-2">
                                <div class="card-body">
                                    <div class="border-bottom mnw-200 mb-4 pb-4">
                                        <h3 class="text-body-emphasis mb-0">
                                            Important Information for Your Stay
                                        </h3>
                                    </div>

                                    <ul class="list-unstyled">
                                        <li>
                                            <h5 class="h5">Simple Pricing:</h5>
                                            <p>
                                                Rates are per person, per night on twin-sharing. Additional charges
                                                apply for single occupancy.
                                            </p>
                                        </li>
                                        <li>
                                            <h5 class="h5">Travel Not Included:</h5>
                                            <p> Arrange your own transport to and from the lodge; transfers are not part
                                                of the package.
                                            </p>
                                        </li>
                                        <li>
                                            <h5 class="h5">Kids Discount:</h5>
                                            <p> Children aged 5 to 12 enjoy a 50% discount when staying with parents.
                                            </p>
                                        </li>
                                        <li>
                                            <h5 class="h5">Price Changes:</h5>
                                            <p> Tariffs may change without notice—double-check before booking!</p>
                                        </li>
                                        <li>
                                            <h5 class="h5">Wellness Treatments:</h5>
                                            <p> Experience relaxing Ayurveda treatments by skilled professionals at the
                                                lodge.</p>
                                        </li>
                                        <li>
                                            <h5 class="h5">Check-in Info:</h5>
                                            <p> Bring valid ID for all guests at check-in. Check-out is at 10:30 AM, and
                                                please settle any outstanding bills.</p>
                                        </li>
                                        <li>
                                            <h5 class="h5">Photography Fees: </h5>
                                            <p>Guests are responsible for any camera and lens entry fees for wildlife
                                                photography. </p>
                                        </li>

                                    </ul>

                                </div>
                            </div>
                            <!-- /Reviews -->
                            <div class="card border-0 mt-4 shadow-sm p-xl-2">
                                <div class="card-body">
                                    <div class="border-bottom mnw-200 mb-4 pb-4">
                                        <h3 class="text-body-emphasis mb-0">
                                            Explore Beyond River Tern Lodge
                                        </h3>
                                    </div>
                                    <p>
                                        Here are some nearby attractions to River Tern Lodge, along with their
                                        approximate
                                        distances and travel times </p>

                                    <h5>Kemmangundi:</h5>
                                    <p>Approximately 54 kilometers from the lodge, Kemmangundi is a scenic
                                        hill station once the summer retreat of the Mysore Royals. Depending on road
                                        conditions, the drive typically takes 1.5 hours.</p>
                                    <h5> Hebbe Falls:</h5>
                                    <p> This beautiful waterfall is over 10 kilometers away from Kemmangundi.
                                        Reaching Hebbe Falls involves a trek or a jeep ride from Kemmangundi, which can
                                        take
                                        2 hours.</p>
                                    <h5>Mullayanagiri:</h5>
                                    <p> The highest peak in Karnataka, Mullayanagiri, is approximately 12
                                        kilometers from Chikmagalur. The drive from River Tern Lodge to Mullayanagiri
                                        can
                                        take 2.5 to 3 hours.
                                    </p>
                                    <h5>Kudremukh:</h5>
                                    <p>
                                        Approximately 95 kilometers southwest of Chikmagalur town, Kudremukh is
                                        known for its unique peak resembling a horse's face and is a popular trekking
                                        destination. The journey from River Tern Lodge to Kudremukh can take 2 hours by
                                        car.</p>
                                    <h5>Sringeri:</h5>
                                    <p>A temple town known for its ancient temples and spiritual significance,
                                        Sringeri is approximately 90 kilometers from River Tern Lodge. The drive can
                                        take
                                        around 2 to 2.5 hours.</p>
                                    <h5>Belur:</h5>
                                    <p> Known for its temples dating back to the Hoysala period, Belur is
                                        approximately 104 kilometers from Chikmagalur. The drive from River Tern Lodge
                                        to
                                        Belur can take 2.5 hours.</p>
                                    <h5>Halebeedu:</h5>
                                    <p> Once the capital of the Hoysala Empire, Halebeedu is approximately 95
                                        kilometers from Chikmagalur. The journey from River Tern Lodge to Halebeedu can
                                        take
                                        2 to 2.5 hours.</p>
                                    <h5>Annapoorneshwari Temple in Hornadu:</h5>
                                    <p>
                                        A famous temple believed to have been
                                        established by Adi Shankaracharya, located in Hornadu town. The temple is
                                        approximately 122 kilometers from River Tern Lodge, and the drive can take
                                        around 3
                                        hours.
                                    </p>

                                    <p class="border-top mt-5  pt-4">
                                        Please note that these distances and travel times are approximate and can vary
                                        based
                                        on the mode of transportation and current road conditions.</p>


                                </div>
                            </div>
                        </div>
                        <!-- /Tour -->
                    </div>

                    <div class=" col-12 col-xxl-3 col-xl-4">


                        <!-- Booking -->
                        <aside id="stickyChild" class="sticky-top sticky-top-120 ps-xl-4 ms-xl-2 ps-xxl-0 ms-xxl-0">
                            <div class="card border-0 shadow-sm">
                                <div class="card-body">
                                    <div class="border-bottom  mb-4">
                                        <h3 class="h4 text-center mb-3">Plan Your Trip</h3>

                                    </div>
                                    <div class="border-bottom  ">
                                        <form class="row g-3" method="post" action="thankyou.php">
                                            <div class="col-12">
                                                <div class="mb-2">
                                                    <div class="input-icon-group">
                                                        <label class="input-icon hicon hicon-topic-user "
                                                            for="txtName"></label>
                                                        <input type="text" class="form-control shadow-sm" id="txtName"
                                                            name="txtName" placeholder="Full Name" required>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="col-12">
                                                <div class="mb-2">
                                                    <div class="input-icon-group">
                                                        <label class="input-icon hicon hicon-telephone hicon-bold"
                                                            for="txtPhone"></label>
                                                        <input type="tel" class="form-control shadow-sm" id="txtPhone"
                                                            name="txtPhone" placeholder="Mobile Number" required>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="col-12">
                                                <div class="mb-2">
                                                    <div class="input-icon-group">
                                                        <label class="input-icon hicon hicon-email-envelope hicon-bold"
                                                            for="txtEmail"></label>
                                                        <input type="email" class="form-control shadow-sm" id="txtEmail"
                                                            name="txtEmail" placeholder="Email Address" required>
                                                    </div>
                                                </div>
                                            </div>

                                            <div class="col-12 col-md-6">
                                                <div class="mb-2">
                                                    <div class="input-icon-group">
                                                        <input id="checkIn" type="text" class="form-control shadow-sm"
                                                            name="checkIn" placeholder="Check-in" required>
                                                        <label class="input-icon hicon hicon-menu-calendar hicon-bold"
                                                            for="checkIn"></label>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="col-12 col-md-6">
                                                <div class="mb-2">
                                                    <div class="input-icon-group">
                                                        <input id="checkOut" type="text" class="form-control shadow-sm"
                                                            name="checkOut" placeholder="Check-out" required>
                                                        <label class="input-icon hicon hicon-menu-calendar hicon-bold"
                                                            for="checkOut"></label>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="col-12 col-md-6">
                                                <div class="input-icon-group">
                                                    <label class="input-icon hicon hicon-adults-one"
                                                        for="selAdults"></label>
                                                    <input type="number" class="form-control shadow-sm" id="selAdults"
                                                        name="selAdults" placeholder="Adults" min="1" required>
                                                </div>
                                            </div>
                                            <div class="col-12 col-md-6">
                                                <div class="input-icon-group">
                                                    <label class="input-icon hicon hicon-child-line hicon-bold"
                                                        for="selchildren"></label>
                                                    <input type="number" class="form-control shadow-sm" id="selchildren"
                                                        name="selchildren" placeholder="Childs" min="1" required>
                                                </div>
                                            </div>
                                            <hr>
                                            <div class="col-12 col-md-12 d-flex justify-content-center">
                                                <div class="g-recaptcha"
                                                    data-sitekey="6Lc2vcoqAAAAAKIJAOEq-i52UQ2DmuuliLmA1Otq"></div>
                                            </div>
                                            <div class="col-12">
                                                <div class="mb-0">
                                                    <button type="submit" class="btn btn-primary btn-uppercase w-100">
                                                        <i class="hicon hicon-bold hicon-menu-bookings"></i>
                                                        <span>SUBMIT NOW</span>
                                                    </button>
                                                </div>
                                            </div>
                                        </form>
                                    </div>
                                </div>
                            </div>
                        </aside>
                        <!-- /Booking -->
                    </div>

                </div>
            </div>
        </section>
        <!-- /Tour details -->


    </main>
    <!-- /Main -->

    <!-- Footer -->
    <footer class="footer">

        <!-- Footer top -->
        <div class="footer-top">
            <div class="container">
                <div class="row">
                    <div class="col-12 col-xl-4 me-5  col-md-6 ">
                        <!-- Brand -->
                        <div class="footer-widget ">
                            <a href="index.php" class="brand-img">
                                <!-- <img  width="100%" height="100%"    sizes="(max-width: 600px) 480px, (max-width: 992px) 768px, 1200px"  class="me-4" src="assets/img/logos/footer-Logo.webp" alt="RiverTern"> -->
                                <h4 class="h2">River Tern Lodge Bhadra</h4>
                            </a>
                            <p class="brand-desc text-white ">Reconnect with nature at Rivertern Lodge, Bhadra. Peaceful surroundings, exciting adventures, and cozy comfort—your perfect escape is here.
                            </p>
                            <h4 class="text-white">Secure Online Booking</h4>
                            <ul class="social-list">
                                <li class="social-item">
                                    <img width="100%" height="100%" sizes="(max-width: 600px) 480px, (max-width: 992px) 768px, 1200px" src="assets/img/FooteSVG/paypal.svg" alt="payment-icons">
                                </li>
                                <li class="social-item">
                                    <img width="100%" height="100%" sizes="(max-width: 600px) 480px, (max-width: 992px) 768px, 1200px" src="assets/img/FooteSVG/mastercard.svg" alt="payment-icons">
                                </li>
                                <li class="social-item">
                                    <img width="100%" height="100%" sizes="(max-width: 600px) 480px, (max-width: 992px) 768px, 1200px" src="assets/img/FooteSVG/visa.svg" alt="payment-icons">
                                </li>
                                <li class="social-item">
                                    <img width="100%" height="100%" sizes="(max-width: 600px) 480px, (max-width: 992px) 768px, 1200px" src="assets/img/FooteSVG/american-express.svg" alt="payment-icons">
                                </li>
                                <li class="social-item">
                                    <img width="100%" height="100%" sizes="(max-width: 600px) 480px, (max-width: 992px) 768px, 1200px" src="assets/img/FooteSVG/rupay.svg" alt="payment-icons">
                                </li>
                            </ul>
                        </div>
                        <!-- /Brand -->
                    </div>

                    <div class="col-12 col-xl-2  col-md-6">
                        <!-- Quick Links -->
                        <div class="footer-widget">
                            <h4 class="h4 text-white pb-2">Navigation</h4>
                            <ul class="footer-link">
                                <li class="link-item">
                                    <a href="about.php">About River Tern</a>
                                </li>
                                <li class="link-item">
                                    <a href="gallery.php">Photo Gallery</a>
                                </li>
                                <li class="link-item">
                                    <a href="things-to-do.php">Things To Do </a>
                                </li>
                                <li class="link-item">
                                    <a href="reach.php">How To Reach</a>
                                </li>

                            </ul>
                        </div>
                        <!-- /Quick Links -->
                    </div>
                    <div class="col-12 col-xl-2 me-5 col-md-6  ">
                        <!-- Quick Links -->
                        <div class="footer-widget">
                            <h4 class="h4 text-white pb-2">Accommodation</h4>
                            <ul class="footer-link">
                                <li class="link-item">
                                    <a href="loghut.php">Loghut Package</a>
                                </li>
                                <li class="link-item">
                                    <a href="cottage.php">Cottage Package</a>
                                </li>
                                <li class="link-item">
                                    <a href="contact-us.php">Contact Us</a>
                                </li>
                                <li class="link-item">
                                    <a href="cancellation.php">Cancellation Policy</a>
                                </li>
                            </ul>
                        </div>
                        <!-- /Quick Links -->
                    </div>
                    <div class="col-12 col-xl-3 col-md-6">
                        <div class="footer-widget">
                            <h4 class="h4 text-white pb-2"> Get In Touch </h4>
                            <!-- Mobile App -->

                            <div class="footer-local">

                                <div class="col-1">
                                    <i class="hicon hicon-map-entry"></i>

                                </div>
                                <div class="col-10">
                                    <p class="text-white">Rivertern Lodge Bhadra, <br>
                                        Behind Ranganathaswamy <br>
                                        Temple, Tarikere, Lakkavalli,<br>
                                        Karnataka 577115</p>
                                </div>

                            </div>


                            <!-- /Mobile App -->
                            <!-- Social -->
                            <div class="footer-local ">

                                <div class="col-1 footer-contact-main ">
                                    <i class="hicon hicon-telephone "></i>
                                    <i class="hicon hicon-email-envelope "></i>
                                </div>
                                <div class=" footer-contact  col-8">
                                    <a href="tel:+9480022108">+91-9480022108</a>
                                    <a href="mailto: book@riverternlodgebhadra.com"> book@riverternlodgebhadra.com
                                    </a>
                                </div>

                            </div>
                            <!-- /Social -->
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <!-- /Footer top -->

        <!-- Footer Bottom -->
        <div class="footer-bottom">
            <div class="container">

                <div class="row">

                    <div class="col-12 col-md-6">
                        <p class="mb-lg-0">© <span class="me-2" id="currentYear"></span> <span id="footer-mark"><a
                                    href="index.php">RiverternLodgeBhadra.com.</a></span> All rights reserved.</p>
                    </div>

                    <div class="col-12 col-md-6">
                        <div class="text-start text-md-end">
                            <ul class="list-inline mb-lg-0">
                                <li class="list-inline-item">
                                    <a href="terms.php">Terms and Conditions</a>
                                </li>
                                <li class="list-inline-item">
                                    <a href="privacy.php">Privacy Policy</a>
                                </li>
                            </ul>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <!-- /Footer Bottom -->

    </footer>
    <!-- /Footer -->
    <script defer src="assets/js/theme-1.min.js"></script>
    <script defer src="assets/js/theme-2.min.js"></script>
    <script defer src="assets/js/theme-3.min.js"></script>
    <script src="https://cdn.jsdelivr.net/npm/flatpickr"></script>
    <script>
        const checkInPicker = flatpickr("#checkIn", {
            dateFormat: "d-m-Y",
            minDate: "today",
            onChange: function(selectedDates) {

                checkOutPicker.set('minDate', selectedDates[0] ? new Date(selectedDates[0].getTime() + 86400000) : "today");
            }
        });

        const checkOutPicker = flatpickr("#checkOut", {
            dateFormat: "d-m-Y",
            minDate: "today"
        });
    </script>

</body>


</html>